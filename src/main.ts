...
//
// DEBT-738（MSG-3148）：**生产禁兜底 mock**——传输先取后用；取不到只挂
// 「无法连接本地服务」页（＋重试），**绝不挂主应用**（旧版静默兜底演示传输
// ⇒ 任意账号密码都能"登进去"，是假登录）。
...
/** 主应用装配（传输已就绪时调用） */
function bootApp(pinia: Pinia, setup: ClientSetup): void {
...
/** 未连接页（＋重试）：传输取不到时的唯一出口（绝不进主应用） */
function mountOffline(setupError: unknown): void {
  const offline = createApp(OfflineView, {
    detail: setupError instanceof Error ? setupError.message : String(setupError ?? ''),
  })
  offline.use(i18n)
  offline.mount('#app')
  console.warn('[baiz] 无可用传输——已挂「无法连接本地服务」页（生产禁兜底 mock）', setupError)
  const errors = (window as unknown as { __errors?: string[] }).__errors ?? []
  errors.push(`offline: ${String(setupError)}`)
  ;(window as unknown as { __errors?: string[] }).__errors = errors
}
